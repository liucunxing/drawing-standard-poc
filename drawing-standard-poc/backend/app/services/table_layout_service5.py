...
class TableLayoutService4Batch:
	...
	def _split_table_box_by_whitespace(self, gray: Any, bbox: Tuple[int, int, int, int]) -> List[Tuple[int, int, int, int]]:
		try:
			import numpy as np
		except Exception:
			return [bbox]
		...
		has_large_gap = any((valid_segments[i + 1][0] - valid_segments[i][1]) >= split_gap for i in range(len(valid_segments) - 1))
