...
模型配置:
- GPU 可用时: MinerU VLM Engine (vlm-engine backend)
- GPU 不可用时: MinerU Pipeline + struct_eqtable (pipeline backend, fallback)
- 语言: 中文 (ch)
...
# 启用 Qwen 大模型后处理（用于管口表矫正）
from backend.app.services.qween_test import fix_nozzle_table_md

os.environ['MINERU_MODEL_SOURCE'] = 'huggingface'

# 模型后端开关：
#   USE_GPU_VLM = True  -> GPU VLM 模型 (vlm-transformers，需要 accelerate)
#   USE_GPU_VLM = False -> CPU pipeline + struct_eqtable（默认，无需 accelerate）
# backend 值经 do_parse 处理后会去掉 "vlm-" 前缀，底层实际使用 "transformers"
USE_GPU_VLM = False

if USE_GPU_VLM and torch.cuda.is_available():
    MINERU_BACKEND = "vlm-transformers"
    print("[img2md] 使用 GPU VLM backend (vlm-transformers)")
else:
    ...
    if torch.cuda.is_available():
        ...
        print("[img2md] 使用 GPU pipeline + struct_eqtable (CUDA 已启用)")
    else:
        print("[img2md] 使用 CPU pipeline + struct_eqtable (CUDA 不可用)")
...
def _parse_pdf_to_md_json(do_parse, pdf_bytes, temp_dir, pdf_name):
    ...
    print(f"[img2md] 开始 do_parse, pdf_name={pdf_name}, output_dir={temp_dir}")
    try:
        do_parse(
        ...
        print(f"[img2md] do_parse 异常: {type(e).__name__}: {e}")
        traceback.print_exc()
        raise

    print(f"[img2md] do_parse 完成, 扫描输出文件...")
    md_files = list(temp_dir.glob("**/*.md"))
    json_files = list(temp_dir.glob("**/*.json"))
    print(f"[img2md] 找到 {len(md_files)} 个 md 文件, {len(json_files)} 个 json 文件")
    if not md_files:
        # 列出 temp_dir 下所有文件帮助调试
        all_files = list(temp_dir.glob("**/*"))
        print(f"[img2md] temp_dir 下所有文件: {[str(f) for f in all_files]}")
