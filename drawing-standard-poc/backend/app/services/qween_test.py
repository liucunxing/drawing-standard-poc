...
# 内网Qwen服务不需要API Key，但新版openai库要求必须传字符串，用占位符即可
API_KEY = "not-needed"
BASE_URL = "http://10.150.18.9:8004/v1"
...
def fix_nozzle_table_md(md_content: str) -> str:
    ...
    try:
        ...
        completion = client.chat.completions.create(
            ...
            temperature=0.0,
            max_tokens=16384  # 16K token，确保模型有足够空间思考并输出修复结果
        )
        ...
        # 检查返回内容是否为空
        if result is None:
            print(f"[Qwen] 警告: API返回内容为None，可能token不足")
            return md_content
